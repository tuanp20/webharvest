...
Extracts products from Redbubble.
"""
...
class RedbubbleExtractor(BaseSiteExtractor):
    """Redbubble Extractor (Tầng 3 - Stealth)."""

    SITE_DOMAIN = "redbubble.com"
    FETCHER_TYPE = "stealth"
    RATE_LIMIT = {"rate": 0.1, "burst": 1}
    ...
    def extract_listing(self, html: str, url: str) -> list[str]:
        ...
        for a in soup.find_all("a", href=True):
            href = a["href"]
            if "/people/" in href and "/shop/" in href:
                full_url = urljoin(url, href)
                # avoid main shop directory
                if not href.endswith("/shop"):
                    if full_url not in links:
                        links.append(full_url)
            elif "/i/" in href or "/shop/p/" in href:
                full_url = urljoin(url, href)
                if full_url not in links:
                    links.append(full_url)
        return links
